...
class BaseRRT:
    ...
    def extract_path(self, goal_idx):
        ...
        depth = 0
        max_depth = len(self.vertices) + 10  # 防止无限循环的最大深度
        ...
        # 如果深度达到最大值但还没结束，可能存在循环
        if depth >= max_depth:
            print(f"警告：提取路径时达到最大深度 {max_depth}，可能存在循环")
        ...
        # 验证路径起点是否与算法起点一致
        if len(path) > 0:
            start_distance = np.linalg.norm(np.array(path[0]) - self.start)
            if start_distance > self.step_size / 2:
                print(f"警告：提取的路径起点 {path[0]} 与算法起点 {self.start.tolist()} 不匹配，距离为 {start_distance}")
                # 如果路径不从起点开始，尝试修复
        ...
        # 验证路径终点是否与终点一致
        if len(path) > 0:
            goal_distance = np.linalg.norm(np.array(path[-1]) - self.goal)
            if goal_distance > self.step_size:
                print(f"警告：提取的路径终点 {path[-1]} 与算法终点 {self.goal.tolist()} 不匹配，距离为 {goal_distance}")
